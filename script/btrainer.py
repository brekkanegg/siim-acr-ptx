...
def trainer(model, optimizer, scheduler, train_loader, val_loader, params, save_dir, device, writer,
    ...
    for epoch in range(params.resume_epoch, params.max_epoch):
        ...
        for i, ((n_fp, n_img, n_seg, n_lbl), (ab_fp, ab_img, ab_seg, ab_lbl)) in enumerate(zip(train_normal_loader, train_abnormal_loader)):
            ...
            # Dice Loss
            dice_criterion = losses.SegmentationLosses(weight=None, batch_weight=batch_weight).build_loss(mode='dice')
            ...
            if params.loss_type == 'wce':
                try:
                    ...
                    ce_pos_weight = None
            ce_criterion = losses.SegmentationLosses(weight=ce_pos_weight, batch_weight=batch_weight).build_loss(mode='bce')
            ce_loss = ce_criterion(outputs, seg)

            # Focal
            focal_criterion = losses.SegmentationLosses(weight=None, batch_weight=batch_weight).build_loss(mode='focal')
...
#################################
            ...
            if it % display_step == 0:
                ...
                visualize.summary_fig(it, save_dir, img.detach(), seg.detach(), outputs.detach(), writer=writer, type='train',)
